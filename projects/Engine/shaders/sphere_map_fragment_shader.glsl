...
vec4 positionalLight(int i) {
	...
	/* Texture Component */
	vec4 SphereTexture = texture2D(SphereMap, out_TextureUV);
	...
	if (DiffuseFactor > 0.0) {
		...
		DiffuseColor = (out_Diffuse * SphereTexture) * LightSources[i].Color * LightSources[i].DiffuseIntensity * DiffuseFactor;
